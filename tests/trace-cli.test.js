...
test('cclens trace export uses web markdown filtering for all traces', async () => {
  const fixture = await createTraceFixture();

  const allOutput = await runTrace([
    'export',
    '--session',
    SESSION_ID,
    '--agent',
  ...
  const allMarkdown = await readFile(allOutput.markdownPath, 'utf8');

  assert.match(allMarkdown, /^# Previous Conversation Context/);
  assert.match(allMarkdown, /\*\*Current filter:\*\* All agents/);
  assert.match(allMarkdown, /\*\*Messages in this export:\*\* 6/);
  assert.match(allMarkdown, /\*Generated by Claude Code Lens\*/);
  assert.match(allMarkdown, /Analyze request latency and find slow tool calls/);
  assert.match(allMarkdown, /Visible tool output/);
  assert.match(allMarkdown, /Idle session summary/);
  assert.doesNotMatch(allMarkdown, /Generate a concise, sentence-case title/);
  assert.doesNotMatch(allMarkdown, /Analyze request latency"\}/);
  assert.doesNotMatch(allMarkdown, />count</);
  assert.doesNotMatch(allMarkdown, /^count$/m);
});
