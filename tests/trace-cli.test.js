...
function lensInputWithSystem(uid, timestamp, content, system) {
  const input = lensInput(uid, timestamp, content);
  input.data.system = system;
  return input;
}
...
async function createTraceFixture() {
  ...
  await writeFile(
    ...
    JSON.stringify({
      ...
      interactions: [
        lensInputWithSystem('title-request', '2026-05-09T09:00:00.000Z', '<session>Analyze request latency</session>', [
          {
            type: 'text',
            text: 'Future Claude Code title prompt text changed completely.'
          }
        ]),
        lensOutput('title-request', '2026-05-09T09:00:01.000Z', [
          {
            type: 'text',
            text: '{"title":"Analyze request latency"}'
          }
        ]),
        lensInput('count-request', '2026-05-09T10:00:01.000Z', 'count'),
        ...
        lensInput('unmatched-request', '2026-05-09T10:05:00.000Z', 'Summarize idle session state.'),
        lensOutput('unmatched-request', '2026-05-09T10:05:01.000Z', [
          {
            type: 'text',
            text: 'Idle session summary.'
          }
        ])
      ]
    })
...
async function createRawLogOnlyFixture() {
  const homeDir = await mkdtemp(path.join(os.tmpdir(), 'cclens-raw-only-home-'));
  const projectsDir = await mkdtemp(path.join(os.tmpdir(), 'cclens-raw-only-projects-'));
  const rawLogsDir = path.join(homeDir, 'raw_logs');
  await mkdir(rawLogsDir, { recursive: true });

  await writeFile(
    path.join(rawLogsDir, 'messages-raw-only.json'),
    JSON.stringify({
      session_id: SESSION_ID,
      created_at: '2026-05-09T11:00:00.000Z',
      interactions: [
        lensInput('raw-lead-request', '2026-05-09T11:00:01.000Z', 'Summarize this raw-only session.'),
        lensOutput('raw-lead-request', '2026-05-09T11:00:02.000Z', [
          {
            type: 'text',
            text: 'Raw-only session summary.'
          }
        ])
      ]
    })
...
test('cclens trace export writes markdown and returns its path plus agent context', async () => {
  ...
  assert.match(markdown, /^# Previous Conversation Context/);
  assert.match(markdown, /\*\*Current filter:\*\* Subagent: trace-worker · Inspect trace timings/);
  assert.match(markdown, /\*\*Messages in this export:\*\* 2/);
  assert.match(markdown, /### Message 1: User/);
  assert.match(markdown, /\*2026-05-09T10:00:19.000Z\*/);
  assert.match(markdown, /### Message 2: Assistant/);
  ...
  assert.doesNotMatch(markdown, /# Claude Code Lens Trace/);
});

test('cclens trace export lead works when native trace is missing', async () => {
  const fixture = await createRawLogOnlyFixture();
  ...
  assert.equal(output.sessionId, SESSION_ID);
  assert.equal(output.agent.id, 'lead');
  assert.equal(output.agent.role, 'lead');
  assert.match(markdown, /\*\*Current filter:\*\* Lead/);
  assert.match(markdown, /\*\*Messages in this export:\*\* 2/);
  assert.match(markdown, /Summarize this raw-only session\./);
  assert.match(markdown, /Raw-only session summary\./);
});

test('cclens trace export does not expose all as a CLI agent target', async () => {
  const fixture = await createTraceFixture();

  await assert.rejects(
    execFileAsync(process.execPath, [cliPath, 'trace', 'export', '--session', SESSION_ID, '--agent', 'all', '-f', 'json'], {
      cwd: repoRoot,
      env: {
        ...process.env,
        CLAUDE_CODE_LENS_HOME: fixture.homeDir,
    ...
    (error) => {
      ...
      assert.equal(payload.error.code, 'AGENT_NOT_FOUND');
      assert.match(payload.error.message, /No agent all found/);
      return true;
    }
  );
});

test('cclens trace export does not expose unmatched as a CLI agent target', async () => {
  const fixture = await createTraceFixture();

  await assert.rejects(
    execFileAsync(process.execPath, [cliPath, 'trace', 'export', '--session', SESSION_ID, '--agent', 'unmatched', '-f', 'json'], {
      cwd: repoRoot,
      env: {
        ...process.env,
        CLAUDE_CODE_LENS_HOME: fixture.homeDir,
    ...
    (error) => {
      ...
      assert.equal(error.code, 66);
      assert.equal(payload.error.code, 'AGENT_NOT_FOUND');
      assert.match(payload.error.message, /No agent unmatched found/);
      return true;
    }
  );
});
